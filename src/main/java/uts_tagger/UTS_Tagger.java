...
public class UTS_Tagger extends AbstractLanguageAnalyser {
    ...
    public void execute() throws ExecutionException {
        ...
        try {
            ...
            if (this.paramOrProperties==ParamOrPropertiesOptions.Properties) {
                if(this.propertiesFile.isEmpty() || !Files.exists(Paths.get(this.propertiesFile))){
                    throw new ResourceInstantiationException("Properties path must be an existing "
                            + "*.properties file");
                }
                api=new UTS_API(this.propertiesFile, this.maxResults);
            } else {
    ...
    @Optional
    @RunTime
    @CreoleParameter(comment = "Properties file location", defaultValue = "")
